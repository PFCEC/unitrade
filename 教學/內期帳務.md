...
使用 daccount.[get_combine]({{ "/API/daccount#get_combine" | relative_url }}) 方法,傳入參數帳號.方式.商品內容和口數

執行後，會回傳一個 [Response]({{ "/API/daccount_data/#response-objects"| relative_url}}) 物件，回覆申請成功或失敗訊息。
```python
...
使用 daccount.[get_net]({{ "/API/daccount#get_net" | relative_url }}) 方法查詢指定帳號
